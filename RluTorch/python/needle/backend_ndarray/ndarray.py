...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        # exceptions
        assert prod(new_shape) == prod(self._shape), "product of current shape is not equal to the product of the new shape"
        ...
        result = self.make(shape=new_shape,strides=new_strides, device=self._device, handle=self._handle, offset=0)
        return result
    ...
    def broadcast_to(self, new_shape):
        ...
        if len(self._shape) == 1:
          return self.make(shape=new_shape, strides=(0,), device=self._device, handle=self._handle, offset=0)

        for i in range(self.ndim):
            if self._shape[i] != 1:
                assert self._shape[i] == new_shape[i], "the broadcast shape does not match the original shape"
    ...
    def process_slice(self, sl, dim):
        ...
        # currently not support negative strides
    ...
    def __getitem__(self, idxs):
        ...
        for i, sl in enumerate(idxs):
            new_shape.append(math.ceil((sl.stop - sl.start) / sl.step))  # get the length for each dim
            new_strides[i] = sl.step * self._strides[i]         # get the stide for each dim
        ...
        result = self.make(shape=new_shape,strides=new_strides, device=self._device, handle=self._handle, offset=new_offset)
        return result
    ...
    def flip(self, axes):
        ...
        og_strides = self._strides
        og_offset = self._offset
        ...
        for i in axes:
          ...
          while n > i:
            temp_offset *= self._shape[n]
            n -= 1
          temp_offset = temp_offset * (self._shape[n] - 1)
          offset += temp_offset
        
        self._offset = offset
        result = self.compact()

        # reverse back to the og setting
        self._strides = og_strides
        self._offset = og_offset
    ...
    def pad(self, axes):
        ...
        for i in range(len(new_shape)):
          new_shape[i] += sum(axes[i])
        
        new_shape = tuple(new_shape)

        # create new NDArray and fill the non-zero sub array
        new_np_array = np.zeros(new_shape, dtype=np.float32)
        slices = []
        for i in range(len(new_shape)):
          slices.append(slice(axes[i][0], new_shape[i] - axes[i][1], 1))
        slices = tuple(slices)

        new_np_array[slices] = self.numpy()
        return NDArray(new_np_array)
...
def amax(a, axis, keepdims=False):
  return a.sum(axis=axis, keepdims=keepdims)
