...
__device__ int32_t getIncompactIdx(size_t gid, size_t size, CudaVec shape, CudaVec strides) {
  int32_t a_idx = 0;
  int32_t curr_idx = 0;
  ...
  for (int32_t i = 0; i < dim; i++) {
    prod = prod / shape.data[i];
    curr_idx = res / prod;
    res = res % prod;

    a_idx += curr_idx * strides.data[i]; 
  }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    int32_t a_idx = getIncompactIdx(gid, size, shape,strides);
    out[gid] = a[offset + a_idx];
  }
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    uint32_t a_idx = getIncompactIdx(gid, size, shape,strides);
    out[offset + a_idx] = a[gid];
  }
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t curr_max = a[reduce_size * gid];
    for (size_t i = 1; i < reduce_size; i++) {
      if (curr_max < a[reduce_size * gid + i])
        curr_max = a[reduce_size * gid + i];
    }
